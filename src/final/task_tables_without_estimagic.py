...
@pytask.mark.depends_on(
    {
        "curve_fit": BLD / "analysis" /'without_estimagic'/ "benchmark_est_pow.yaml", 
        "least_square": BLD / "analysis" /'without_estimagic'/ "benchmark_est_pow_alt1.yaml",
        "minimize_neldermead": BLD / "analysis" /'without_estimagic'/ "benchmark_est_pow_alt2.yaml"
    }
)
...
def task_compare_opt_pow(depends_on, produces):
    ...
    est_dictionary['authors'] = np.round([20.546,0.0,3.17, 672.387], 3)
...
@pytask.mark.depends_on(
    {
        "bench pow": BLD / "analysis" /'without_estimagic'/ "benchmark_est_pow.yaml", 
        "noweight pow": BLD / "analysis" /'without_estimagic'/ "no_weight_est_pow.yaml", 
        "bench exp": BLD / "analysis" /'without_estimagic'/ "benchmark_est_exp.yaml", 
        "noweight exp": BLD / "analysis" /'without_estimagic'/ "no_weight_est_exp.yaml" 
    }
)
...
def task_nls_noweight_behavioral(depends_on, produces):
    ...
    for i in ['pow','exp']:
        ...
        with open(depends_on[f'noweight {i}'], "r") as stream:
            ...
            est_dictionary[f'{i}_se'] += temp['std err'][3:]
...
@pytask.mark.depends_on(
    {
        "lin curv pow": BLD / "analysis" /'without_estimagic'/ "weight_lin_curv_est_pow.yaml", 
        "conc curv pow": BLD / "analysis" /'without_estimagic'/ "weight_conc_curv_est_pow.yaml", 
        "estimated curv pow": BLD / "analysis" /'without_estimagic'/ "weight_est_curv_est_pow.yaml", 
        "lin curv exp": BLD / "analysis" /'without_estimagic'/ "weight_lin_curv_est_exp.yaml",
        "conc curv exp": BLD / "analysis" /'without_estimagic'/ "weight_conc_curv_est_exp.yaml",
        "estimated curv exp": BLD / "analysis" /'without_estimagic'/ "weight_est_curv_est_pow.yaml" 
