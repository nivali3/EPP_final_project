...
def _create_inputs_exp(data):
    ...
    benckmark_params = pd.DataFrame(
        [gamma_init, k_init / k_scaler, s_init / s_scaler], columns=["value"]
    )
    benckmark_params["soft_lower_bound"] = 1e-123
    benckmark_params["soft_upper_bound"] = 40

    no_weight_params = pd.DataFrame(
        np.concatenate(
            (
    ...
    prob_weight_lin_curv_params = pd.DataFrame(
        np.concatenate(
    ...
    prob_weight_conc_curv_params = pd.DataFrame(
        np.concatenate(
            ([gamma_init, k_init / k_scaler, s_init / s_scaler], prob_weight_init)
        ),
        columns=["value"],
    ...
    prob_weight_est_curv_params = pd.DataFrame(
        np.concatenate(
            (
                [gamma_init, k_init / k_scaler, s_init / s_scaler],
                prob_weight_init,
    ...
    benchmark = {
        ...
        "x_data": piece_rates,
        "buttonpresses": np.array(
            data.loc[data["dummy1"] == 1].buttonpresses_nearest_100
        ),
    }
    no_weight = {
        ...
        "buttonpresses": np.array(
            data.loc[data["samplenw"] == 1].buttonpresses_nearest_100
        ),
    }
    prob_weight_lin_curv = {
        ...
        "buttonpresses": np.array(
            data.loc[data["samplepr"] == 1].buttonpresses_nearest_100
        ),
    }
...
def _create_inputs_pow(data):
    ...
    benchmark = {
        "params_init": benckmark_params,
        "x_data": piece_rates,
        "log_buttonpresses": np.array(
            data.loc[data["dummy1"] == 1].logbuttonpresses_nearest_100
        ),
    }
    no_weight = {
        "params_init": no_weight_params,
        "x_data": no_weight_x_data,
        "log_buttonpresses": np.array(
            data.loc[data["samplenw"] == 1].logbuttonpresses_nearest_100
        ),
    }
    prob_weight_lin_curv = {
        "params_init": prob_weight_lin_curv_params,
        "x_data": prob_weight_x_data,
        "log_buttonpresses": np.array(
            data.loc[data["samplepr"] == 1].logbuttonpresses_nearest_100
        ),
    }
    prob_weight_conc_curv = {
        "params_init": prob_weight_conc_curv_params,
...
def pow_bootstrap_estimagic(data, scenario, algorithm):
    ...
    _estimagic = partial(
        sqrd_residuals_estimagic,
        xdata=x_data,
        buttonpresses=log_buttonpresses,
        scenario=scenario,
    )
...
specifications_exp = (
    (
        ...
        / f"{scenario}_est_exp_estimagic_{algorithm}.yaml",
        scenario,
        algorithm,
    )
    for scenario in [
...
specifications_pow = (
    (
        ...
        / "analysis"
        / "estimated_parameters_estimagic"
        / f"{scenario}_est_pow_estimagic_{algorithm}.yaml",
        scenario,
        algorithm,
    )
    for scenario in [
...
def task_estimate_params_exp_estimagic(produces, scenario, algorithm):
    ...
    _bootstrap = partial(
        exp_bootstrap_estimagic, scenario=scenario, algorithm=algorithm
    )
...
def task_estimate_params_pow_estimagic(produces, scenario, algorithm):
    ...
    _bootstrap = partial(
        pow_bootstrap_estimagic, scenario=scenario, algorithm=algorithm
    )
