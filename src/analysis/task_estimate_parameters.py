...
from src.model_code.model_function import sqrd_residuals_benchmark
...
def _create_inputs_exp(data):
    ...
    no_weight_x_data = {
        "payoff_per_100": data.loc[data["samplenw"] == 1].payoff_per_100,
        ...
        "payoff_charity_per_100": data.loc[
            data["samplenw"] == 1
        ].payoff_charity_per_100,
        "charity_dummy": data.loc[data["samplenw"] == 1].charity_dummy,
    }
    prob_weight_x_data = {
        "payoff_per_100": data.loc[data["samplepr"] == 1].payoff_per_100,
    ...
    prob_weight_lin_curv = {
        "params_init": np.concatenate(
            ([gamma_init, k_init / k_scaler, s_init / s_scaler], prob_weight_init)
        ),
...
@pytask.mark.parametrize(
    ...
    [
        (
            BLD / "analysis" / "estimated_parameters" / "benchmark_est_exp.yaml",
            "benchmark",
        ),
        (
            BLD / "analysis" / "estimated_parameters" / "no_weight_est_exp.yaml",
            "no_weight",
        ),
        (
            BLD / "analysis" / "estimated_parameters" / "weight_lin_curv_est_exp.yaml",
            "prob_weight_lin_curv",
        ),
        (
            BLD / "analysis" / "estimated_parameters" / "weight_conc_curv_est_exp.yaml",
            "prob_weight_conc_curv",
        ),
        (
            BLD / "analysis" / "estimated_parameters" / "weight_est_curv_est_exp.yaml",
            "prob_weight_est_curv",
        ),
    ],
)
def task_estimate_params_exp(depends_on, produces, scenario):
    ...
    sol = opt.curve_fit(_partial, x_data, buttonpresses, params_init)
    est = sol[0]
    std_errors = np.sqrt(np.diagonal(sol[1]))

    final = {"estimates": est.tolist(), "std dev": std_errors.tolist()}
...
@pytask.mark.parametrize(
    ...
    [
        (
            BLD / "analysis" / "estimated_parameters" / "benchmark_est_pow.yaml",
            "benchmark",
        ),
        (
            BLD / "analysis" / "estimated_parameters" / "no_weight_est_pow.yaml",
            "no_weight",
        ),
        (
            BLD / "analysis" / "estimated_parameters" / "weight_lin_curv_est_pow.yaml",
            "prob_weight_lin_curv",
        ),
        (
            BLD / "analysis" / "estimated_parameters" / "weight_conc_curv_est_pow.yaml",
            "prob_weight_conc_curv",
        ),
        (
            BLD / "analysis" / "estimated_parameters" / "weight_est_curv_est_pow.yaml",
            "prob_weight_est_curv",
        ),
    ],
)
def task_estimate_params_pow(depends_on, produces, scenario):
    ...
    sol = opt.curve_fit(_partial, x_data, log_buttonpresses, params_init)
    est = sol[0]
    std_errors = np.sqrt(np.diagonal(sol[1]))
    ...
    final = {
        ...
        "min obj func": (
            np.sum((_partial(x_data, *est) - log_buttonpresses) ** 2)
        ).tolist(),
    }
    with open(produces, "w") as y:
        yaml.dump(final, y)
...
def task_estimate_params_benchmark_pow_alt1(depends_on, produces):
    ...
    sol = opt.least_squares(
        lambda params: sqrd_residuals_benchmark(
            params,
            xdata=x_data,
            logbuttonpresses=log_buttonpresses,
            optimizer="opt.least_squares",
        ),
        params_init,
        xtol=1e-15,
        ftol=1e-15,
