...
def test_estimated_effort_prob_weight_lin_curv():
    ...
    expected_est_effort = pd.Series(
        data=[38.89874011709243, 38.93925328608813, 38.89874011709243]
    )
    actual_est_effort = estimated_effort(
        x_data, *params, scenario="prob_weight_lin_curv"
    )
...
def test_estimated_effort_prob_weight_conc_curv():
    ...
    expected_est_effort = pd.Series(
        data=[38.908706711223445, 38.964704404887044, 38.90303953691895]
    )
    actual_est_effort = estimated_effort(
        x_data, *params, scenario="prob_weight_conc_curv"
    )
...
def test_estimated_effort_prob_weight_est_curv():
    ...
    expected_est_effort = pd.Series(
        data=[39.02018127785867, 39.15477086114424, 38.927943660188205]
    )
    actual_est_effort = estimated_effort(
        x_data, *params, scenario="prob_weight_est_curv"
    )
...
def _create_test_inputs():
    ...
    no_weight = {
        "params": np.concatenate(([gamma, k, s], [alpha, a, gift, beta, delta])),
        "x_data": no_weight_x_data,
        "buttonpresses": pd.Series(data=[25, 100, 200]),
    }
    prob_weight_lin_curv = {
        "params": np.concatenate(([gamma, k, s], prob_weight)),
    ...
    prob_weight_est_curv = {
        "params": np.concatenate(([gamma, k, s], prob_weight, curv)),
        "x_data": prob_weight_x_data,
        "buttonpresses": pd.Series(data=[25, 100, 200]),
    }
