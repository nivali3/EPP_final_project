...
def create_nls_data(data, treatment_classes, payoff_classes, scenario_dummies):
    ...
    return pd.concat(
        objs=[data, data_with_payoffs, data_with_efforts, data_with_dummies], axis=1
    )
...
def _create_efforts(data):
    ...
    data_with_efforts.loc[
        data_with_efforts["buttonpresses_nearest_100"] == 0, "buttonpresses_nearest_100"
    ] = 25
    data_with_efforts["logbuttonpresses_nearest_100"] = np.log(
        data_with_efforts["buttonpresses_nearest_100"]
    )

    return data_with_efforts


def _create_scenario_dummies(data, scenario_dummies):

    data_with_dummies = data[["treatment"]]

    for i in scenario_dummies:
        data_with_dummies[i] = 0
        if i == "dummy1":
            for j in scenario_dummies[i]:
                data_with_dummies[i] += (data_with_dummies["treatment"] == j).astype(
                    int
                )
        else:
